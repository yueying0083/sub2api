export default {
  promptAudit: {
    ...
    tabs: { config: '配置', events: '事件', activity: '用户工作记录' },
    ...
    saveBar: { enabled: '启用提示词审计', activityRecording: '记录用户输入（不阻断）', blocking: '同步阻止', blockingLatestTurnOnly: '仅审最新输入和上一轮输出', storePass: '保存安全事件', dirty: '有未保存的更改', synced: '配置已同步' },
    blockingConfirm: {
      title: '开启同步阻止？',
    ...
    activity: {
      title: '用户工作记录',
      description: '仅汇总每次请求中的最新用户输入；不保存系统提示词或历史上下文，也不参与请求阻断、调度或计费。',
      incompleteNotice: '当前未开启“保存安全事件”，这里只能统计命中风险的用户输入，无法完整反映成员工作记录。',
      totalUsers: '活跃成员', totalPrompts: '用户输入记录', activeDays: '团队活跃天数',
      keyword: '成员或用户输入', keywordHint: '用户名、邮箱或输入预览', member: '成员', promptCount: '输入次数',
      memberActiveDays: '活跃天数', lastActive: '最近活跃', modelsAndGroups: '模型 / 分组', latestPrompt: '最近用户输入',
      viewRecords: '查看输入记录', exportCsv: '按时间导出 CSV', empty: '所选范围内没有用户输入记录。',
      pageSummary: '第 {page} / {pages} 页 · 共 {total} 名成员',
    },
    events: {
    ...
    errors: {
      loadConfig: '无法加载提示词审计配置。', loadRuntime: '无法加载提示词审计运行态。', loadGroups: '无法加载分组列表。', loadEvents: '无法加载审计事件。', loadActivity: '无法加载用户工作记录。', exportActivity: '无法导出用户输入记录。', loadDetail: '无法加载事件详情。', saveConfig: '配置保存失败。', probe: '节点探测失败。', delete: '事件删除失败。', previewDelete: '无法生成删除预览，请检查时间范围。', deleteConfirmation: '删除确认无效或已过期，请重新预览。',
